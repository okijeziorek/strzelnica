...
$hero_url = $hero_id ? wp_get_attachment_image_url($hero_id, 'full') : get_template_directory_uri() . '/hero.jpg';
...
<section class="feature-grid" id="pakiety" aria-label="<?php echo esc_attr(gro_dynamic_setting('gro_features_label')); ?>">
    <?php foreach ($features as $index => $item) : ?>
        <article class="feature-card">
            <div class="line-icon">
                <?php if (has_post_thumbnail($item)) : echo get_the_post_thumbnail($item, 'thumbnail'); else : echo gro_dynamic_feature_icon($index); endif; ?>
            </div>
